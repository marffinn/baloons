...
let load_consecutive = function (objectFetch, nameFetch, materialFetch, familyFetch, mainName, dimensions) {
  ...
  let model_scale = dimensions[0][1];
  ...
  loader.load(objectFetch, function (object) {
    ...
    // objectile.scale.set(model_scale,model_scale,model_scale);
...
function animIn(model_name, model_scale) {

  if( model_scale == null ){
    console.log('siemka NULL');
  }
...
let ms = .2;
// loader.load('assets/DUDE/miara.json', function (geometry) {
//   meter = new THREE.Mesh(geometry);
//   meter.position.set(0, 0, 0);
//   meter.scale.set(ms,ms,ms);
//   meter.name = 'meter';
//   group_help.add(meter);
// });
...
let models = {
  ...
  'b12': {
    ...
    dimensions: [['2m', 0.5] , ['3m', 0.75], ['4m', 1], ['6m', 2.2], ['8m', 2.2], ['10m', 2.2]],
  },
  'b16': {
    name: 'Balon B16',
    family: ['balon', 'b12', 'b16', 'grzybek'],
