...
function animIn(model_name) {
  ...
  // Prepare functional arrays for new model
  cookieModel = model_name;
  raycastArray = [];
  chosenArray = {};
  ///////////////////////////////////////////////////////////////
  
  // Remove group contents  /////////////////////////////////////
  ...
  // Load model parts
  sizePopulate(models[model_name].family);
  dimensionPopulate(models[model_name].dimensions);
  for (i = 0; i <= models[model_name].arya.length - 1; i++) {
    let mates = new THREE.MeshLambertMaterial({
      color: models[model_name].arya[i][2],
      side: THREE.DoubleSide
    });
    $('.product-name').text(models[model_name].name);
    load_consecutive(models[model_name].arya[i][0], models[model_name].arya[i][1], mates, models[model_name].family, models[model_name].name);
  }
  ///////////////////////////////////////////////////////////////
