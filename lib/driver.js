...
$("#modal").dmUploader({
  ...
  onUploadSuccess: function(id, data) {
    ...
    Cookies.set('x_picturesUploaded', imager, { expires: 7 });

  }
});

let sizePopulate = function(familyFetch) {
  $('.sizeSelector').selectric('destroy').empty();
  for (i = 0; i <= familyFetch.length - 1; i++) {
    let checkDataO = '<option value="' + familyFetch[i] + '">' + familyFetch[i] + '</option>';
    $('.sizeSelector').append(checkDataO);
  }
  $('.sizeSelector').selectric();

  // selectric here to change the apended state so list resembles the model chosen
  $('.selectric .label').text( cookieModel );
  $('.selectric-scroll ul li ').each(function(){
    $(this).removeClass('selected');
  });
  $('.selectric-scroll ul li:contains('+ cookieModel +')').addClass('selected');
