...
$('.sdodatki a.clr').on('click', function() {
  ...
  if( $(this).hasClass('check') ){
    $(this).toggleClass('check');
    Cookies.set(option, 'no', {
      expires: 7
    });
  } else {
    $(this).toggleClass('check');
    Cookies.set(option, 'yes', {
      expires: 7
    });
  }
...
let checkAddons = function(){
  $('.sdodatki a').each(function(){
    if( $(this).hasClass('check') ){
      ...
      // let optionGlued = { $(this).text() : 'yes'};
      // addinsArray.push( $(this).text );

      Cookies.set(cookieModel, addinsArray, {
        expires: 7
      });
      console.log( $(this).text() + ' pushed' );
    } else {
      return 0;
    }
  });
...
let centerSteering = function(){
  ...
  $('.steering').css({'margin-left':  -(steeringWidth/2)  });
}
centerSteering();
...
let linkInterpreter = function(link) {
  ...
  $.each(retrievedArray, function(partName, colorToSet) {
    ...
    let option = partName + ' : ' + colorToSet;
