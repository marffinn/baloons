...
$(document).on('click', '.skolory a:not(".clr")', function(){
...
$(document).on('click', '.skolory a.clr', function(){
  ...
  if (window.location.hash) {
    ...
    $.each(retrievedArray.addins, function (v, k) {
      if (k == 'yes') {
        ...
        $('.skolory .clr:contains(' + v + ')').toggleClass('check');
      } else {
        $('.sdodatki a').removeClass('check');
        $('.skolory a .clr').removeClass('check');
      }
    });
...
$('.sendMail').on('click', function () {
  ...
  if( $(this).hasClass('sendDisable') ){
    displayRodo();
  } else {

    $(this).toggleClass('sendDisable');
    Email.send({
